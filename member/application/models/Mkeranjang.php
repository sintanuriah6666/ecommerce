...
class Mkeranjang extends CI_Model {
    ...
    function simpan($inputan, $id_produk) {
        // Mendapatkan detail produk berdasarkan id_produk
        $this->db->where('id_produk', $id_produk);
        $ap = $this->db->get('produk');
        $dp = $ap->row_array();
        
        // Menyiapkan data yang akan dimasukkan ke keranjang
        $inputan['id_produk'] = $id_produk;
        $inputan['id_member_beli'] = $this->session->userdata('id_member');
        $inputan['id_member_jual'] = $dp['id_member'];
        
        // Cek apakah member ini sudah memiliki produk ini di keranjang
        $this->db->where('id_produk', $id_produk);
        ...
        if (empty($dk)) {
            ...
            // Jika sudah ada, maka update data yang ada
            $this->db->where('id_produk', $id_produk);
            $this->db->where('id_member_beli', $this->session->userdata('id_member'));
    ...
    function checkout($keranjang, $member_jual, $member_beli, $nama_ekspedisi, $ongkirterpilih) {
        ...
        $m['bayar_transaksi'] = $totalbelanja + $ongkirterpilih['cost'][0]['value']; // atau nilai lain yang sesuai
